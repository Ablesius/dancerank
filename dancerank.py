...
os.environ['STANFORD_MODELS'] = './stanford-ner-2015-04-20/stanford-german-2015-01-30-models' \
...
os.environ['STANFORD_PARSER'] = './stanford-ner-2015-04-20/'
...
#              ["""
# these uncommented lines are actually only needed for debugging, and can be safely removed
# TATATA
#
...
#             """, "-o", "/tmp/test.csv"]
    ...
    if parse_arguments.output:
        if os.path.isfile(_output):
            global opentype
            opentype = input("File 'output' exists. Would you like to (o)verwrite, (a)ppend, or (c)ancel? ")
            if opentype == 'a' or opentype == 'append':
                print("Appending to file.")
                outputfile = open(_output, 'a')
            elif opentype == 'o' or opentype == 'overwrite':
                print("Overwriting file.")
                outputfile = open(_output, 'w')
            else:
                print("Cancelling programme.")
                exit(1)
        else:
            opentype = "w"
            outputfile = open(_output, 'w')
